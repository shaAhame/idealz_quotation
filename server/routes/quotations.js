...
// GET /quotations — list (scoped to branch for managers)
...
// GET /quotations/stats — dashboard stats
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    if (!clientName || !clientEmail || !taxMode || !Array.isArray(items) || !items.length) {
      return res.status(400).json({ error: 'clientName, clientEmail, taxMode, and items are required' });
    }
...
// GET /quotations/:id/pdf — download PDF (authenticated)
router.get('/:id/pdf', auth, async (req, res) => {
  try {
    ...
    const baseUrl = process.env.APP_URL || `${req.protocol}://${req.get('host')}`;
    const pdf = await generatePDF(q, baseUrl);
    res.set({
