...
export function QuotationDetail() {
  ...
  const { sscl, vat, total } = calcTax(q.subTotal, q.taxMode);
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
        <div className="totals-box">
          ...
          {q.taxMode === 'VAT18_SSCL25' &&
            <div className="total-line"><span>SSCL 2.5%</span><span>{fmtRs(q.ssclAmount)}</span></div>}
          {(q.taxMode === 'VAT18' || q.taxMode === 'VAT18_SSCL25') &&
            <div className="total-line"><span>VAT 18%</span><span>{fmtRs(q.vatAmount)}</span></div>}
          {q.taxMode === 'FLAT205' &&
            <div className="total-line"><span>Tax 20.5%</span><span>{fmtRs(q.total - q.subTotal)}</span></div>}
          {q.taxMode === 'VAT_INCLUSIVE' &&
            <div className="total-line"><span>Incl. VAT</span><span>{fmtRs(q.vatAmount)}</span></div>}
